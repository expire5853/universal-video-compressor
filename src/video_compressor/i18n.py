...
ZH_CN_TRANSLATIONS: dict[str, str] = {
    ...
    "1  Select a video  ·  2  Finish hardware detection  ·  3  Choose a quick profile  ·  4  Start compression": "1  选择视频  ·  2  完成硬件检测  ·  3  选择快速方案  ·  4  开始压制",
    ...
    "Video codec": "视频编码",
    "Container": "封装格式",
    "Only CPU and GPU devices that passed a real FFmpeg encoding test are listed. NPU information is shown as detection status only.": "这里只列出已通过真实 FFmpeg 编码测试的 CPU 和 GPU 设备。NPU 信息仅作为检测状态显示。",
    "Friendly codec names are shown here. Technical FFmpeg encoder names remain available in Detection details.": "此处显示易懂的编码格式名称；FFmpeg 技术编码器名称仍可在“检测详情”中查看。",
    "Only verified encoding choices appear above. Unavailable devices, drivers, and technical test results remain in Detection details.": "上方只显示已验证可用的编码选项。不可用设备、驱动和技术测试结果仍保留在“检测详情”中。",
    "No verified encoder available": "没有已验证可用的编码设备",
    "No verified codec available": "没有已验证可用的编码格式",
    ...
    " seconds": " 秒",
    "Quick profile": "快速方案",
    "Show advanced video settings": "显示高级视频设置",
    "Hide advanced video settings": "隐藏高级视频设置",
    "Quick profiles already configure these options. Expand them only when you need manual control.": "快速方案已经配置好这些选项；仅在需要手动控制时展开。",
    "Quality mode": "质量类型",
    ...
    "{backend}: {count} codec options": "{backend}：{count} 种编码格式",
    "Verified for compression: {backends}": "已验证可用于压制：{backends}",
    "No CPU or GPU encoder passed the initialization test.": "没有 CPU 或 GPU 编码器通过初始化测试。",
    "Unavailable CPU/GPU backends are hidden; open Detection details for the device, driver, or initialization reason.": "不可用的 CPU/GPU 后端已隐藏；可打开“检测详情”查看设备、驱动或初始化失败原因。",
    "NPU detected (driver {version}); status only, because no FFmpeg video encoder is available.": "已检测到 NPU（驱动 {version}）；由于没有可用的 FFmpeg 视频编码器，因此仅显示状态。",
    "NPU not detected; NPU is status-only and is not offered as an encoding device.": "未检测到 NPU；NPU 仅作为状态信息，不会作为编码设备提供。",
    "; ": "；",
    "{backends}\n{npu}.": "{backends}\n{npu}。",
