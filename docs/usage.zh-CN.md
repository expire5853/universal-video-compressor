...
编码器存在于 FFmpeg 列表中并不代表可用。驱动初始化、实际输出、位深、尺寸、封装或回读校验失败时，对应 CPU/GPU 后端不会出现在主选项框中。可以打开“检测详情”查看所有已检测设备、技术编码器名称和失败原因。当前 FFmpeg 集成没有 NPU 视频编码器，因此 NPU 检测仅显示状态。
...
主编码格式选项框使用易懂的名称，FFmpeg 技术编码器名称保留在“检测详情”中。快速方案会保留最常用的选项；质量类型/数值、速度、像素位深和关键帧间隔默认收纳在“显示高级视频设置”下。界面会明确提示数值方向，并动态过滤当前编码器和位深不支持的模式。

其他选项包括分辨率、1–240 fps、音频复制、AAC、Opus、FLAC、移除音频以及音频码率。
